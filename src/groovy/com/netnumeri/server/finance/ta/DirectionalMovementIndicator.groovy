...
public class DirectionalMovementIndicator implements Serializable {
    ...
    public static int classifyMovements(double todayHigh,
                                        double todayLow,
                                        double yesterdayHigh,
                                        double yesterdayLow) {
        if (todayHigh < 0.0D) throw new IllegalArgumentException("Todays high (i.e. todaysHigh) must be a positive number.");
        if (todayLow < 0.0D) throw new IllegalArgumentException("Todays low (i.e. todaysLow) must be a positive number.");
        if (yesterdayLow < 0.0D)
            throw new IllegalArgumentException("Yesterdays low (i.e. yesterdaysLow) must be a positive number.");
        if (yesterdayHigh < 0.0D)
            throw new IllegalArgumentException("Yesterdays high (i.e. yesterdaysHigh) must be a positive number.");
        if (todayHigh > yesterdayHigh && todayLow > yesterdayLow) return 1;
        if (todayHigh < yesterdayHigh && todayLow < yesterdayLow) return 2;
        if (todayHigh > yesterdayHigh && todayLow > yesterdayHigh) return 3;
        if (todayHigh < yesterdayLow && todayLow < yesterdayLow) return 4;
        return todayHigh < yesterdayHigh || todayLow > yesterdayLow ? 6 : 5;
    }

    public static double positiveDirectionalMovement(double todayHigh,
                                                     double todayLow,
                                                     double yesterdayHigh,
                                                     double yesterdayLow) {
        if (todayHigh < 0.0D) throw new IllegalArgumentException("Todays high (i.e. todaysHigh) must be a positive number.");
        ...
        if (todayHigh > yesterdayHigh && todayLow > yesterdayLow) {
            return todayHigh - yesterdayHigh;
        }
        if (todayHigh > yesterdayHigh && todayLow > yesterdayHigh) {
            return todayHigh - yesterdayHigh;
        }
        if (todayHigh >= yesterdayHigh && todayLow <= yesterdayLow && todayHigh - yesterdayHigh > yesterdayLow - todayLow) {
            return todayHigh - yesterdayHigh;
        } else {
            return 0.0D;
        }
    ...
    public static double minusDirectionalMovement(double todayHigh,
                                                  double todayLow,
                                                  double yesterdayHigh,
                                                  double yesterdayLow) {
        if (todayHigh < 0.0D) throw new IllegalArgumentException("Todays high (i.e. todaysHigh) must be a positive number.");
        ...
        if (yesterdayHigh < 0.0D)
            throw new IllegalArgumentException("Yesterdays high (i.e. yesterdaysHigh) must be a positive number.");
        if (todayHigh < yesterdayHigh && todayLow < yesterdayLow) {
            return yesterdayLow - todayLow;
        }
        if (todayHigh < yesterdayLow && todayLow < yesterdayLow) {
            return yesterdayLow - todayLow;
        }
        if (todayHigh >= yesterdayHigh && todayLow <= yesterdayLow && yesterdayLow - todayLow > todayHigh - yesterdayHigh) {
            return yesterdayLow - todayLow;
        } else {
            return 0.0D;
        }
    ...
    public static double trueRange(double todayHigh, double todayLow, double yesterdaysClose) {
        if (todayHigh < 0.0D) throw new IllegalArgumentException("Todays high (i.e. todaysHigh) must be a positive number.");
        if (todayLow < 0.0D) throw new IllegalArgumentException("Todays low (i.e. todaysLow) must be a positive number.");
        if (yesterdaysClose < 0.0D)
            throw new IllegalArgumentException("Yesterdays close (i.e. yesterdaysClose) must be a positive number.");
        else {
            double d3 = Math.max(todayHigh, yesterdaysClose);
            double d4 = Math.min(todayLow, yesterdaysClose);
            return d3 - d4;
        }
    }
    ...
    public static double[] plusDirectionalMovementPeriod(double[] high, double[] low, double[] close) {
        if (high.length != low.length || low.length != close.length || close.length != high.length) {
            throw new IllegalArgumentException("The arrays high, low and close must have the same length");
        }
        double[] ad3 = new double[high.length - 1];
        for (int i = 0; i < high.length - 1; i++) {
            ad3[i] = plusDirectionalMovement(high[i], low[i], high[i + 1], low[i + 1], close[i + 1]);
        }
    ...
    public static double[] minusDirectionalMovementPeriod(double[] high, double[] low, double[] close) {
        if (high.length != low.length || low.length != close.length || close.length != high.length) {
            throw new IllegalArgumentException("The arrays high, low and close must have the same length");
        }
        double[] ad3 = new double[high.length - 1];
        for (int i = 0; i < high.length - 1; i++) {
            ad3[i] = minusDirectionalMovement(high[i], low[i], high[i + 1], low[i + 1], close[i + 1]);
        }
    ...
    public static int dmiSignal(double[] ad, double[] ad1, int method)
